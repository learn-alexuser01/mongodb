...
function PostsDAO(db) {
    ...
    this.incrementLikes = function(permalink, comment_ordinal, callback) {
        ...
        this.getPostByPermalink(permalink, function(err, postResult){
            "use strict";
            if (err) return callback(err,null);
            var selector = {},
                strNumlikes = '';

            strNumlikes = 'comments.' + comment_ordinal + '.num_likes';  
            selector[strNumlikes] = 1;          

            posts.update({'permalink': permalink}, {'$inc': selector}, { upsert: true }, function(err, updateResult) {
                "use strict";
                if (err) return callback(err, null);
